...
import { Paperclip, Search, StickyNote } from "lucide-react";
...
/* ---------------------------------------------------------------------------
   The desk: one of the two things in the dock.
   ...
   else to go. So the desk is reachable from every step, and reachable without
   ...
   The tab, the drawer and the way out of it belong to `Dock`, which carries
   this and the running site behind one tab against one edge. This is the
   contents of that drawer, not the drawer.
--------------------------------------------------------------------------- */
...
export function NotesBody({
  ...
  /** Shuts the drawer, for the links inside that take you back to a step. */
  onClose: () => void;
}) {
  ...
  return (
    <>
      ...
      <p className="mt-4 rounded-[12px] bg-well px-3.5 py-2.5 text-[12px] leading-[1.5] text-quiet">
        Filed under <b className="font-semibold text-ink">{whereName(filed)}</b>{" "}
        - it turns up in the document beside that answer, not in a pile at the
        end.
      </p>
      ...
      <div className="mt-3">
        <AddRow
          ...
          onAdd={(value) => addRef({ kind: "Note", text: value, where: filed })}
        />
      </div>
      ...
      {groups.length ? (
        ...
        <p className="note-leaf note-rule mt-5 bg-canvas py-4 pe-4 ps-[26px] text-[12.5px] leading-[1.55] text-quiet">
          Nothing on the desk yet, and that is a finished answer - nothing here
          is required or checked against anything.
        </p>
      )}
      ...
      {count ? (
        <p className="mt-6 border-t border-hair pt-4 text-[12px] leading-[1.55] text-label">
          All {count} go with the scoping request, each one beside the answer it
          was written against. Nothing here is required, and nothing is scored.
        </p>
      ) : null}
    </>
  );
