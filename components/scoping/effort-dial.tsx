"use client";

import { useMemo } from "react";

import {
  buildDial,
  DIAL_VIEWBOX,
  effortRag,
  HUB_RADIUS,
  RAG_COLOUR,
  RAG_LABEL,
  STEP_COUNT,
} from "@/lib/scoping";
import { cn } from "@/lib/utils";

import { useScopingSession } from "./scoping-context";

/**
 * The effort dial: three rings and a hub.
 *
 * Inner ring is the eight sections, middle is each question answered inside one,
 * outer is each component chosen inside that question. Colour is effort, grey is
 * unanswered, and the section in play is the only one at full opacity.
 *
 * The inner ring is clickable, so the dial is navigation as well as a readout.
 * Every segment carries a `<title>`, which is what a screen reader and a native
 * tooltip both read.
 */
export function EffortDial({ className }: { className?: string }) {
  const { answers, index, goTo, percent, overall, complete } =
    useScopingSession();

  const segments = useMemo(
    () => buildDial(answers, index, complete),
    [answers, index, complete],
  );

  const rag = overall === null ? "todo" : effortRag(overall);

  return (
    <div className={cn("relative", className)}>
      <svg
        viewBox={`0 0 ${DIAL_VIEWBOX} ${DIAL_VIEWBOX}`}
        className="block size-full"
        role="img"
        aria-label={`Scoping progress: ${percent} percent complete, overall effort ${RAG_LABEL[rag]}`}
      >
        {segments.map((segment) =>
          segment.stepIndex === undefined ? (
            <path
              key={segment.id}
              d={segment.d}
              fill={segment.fill}
              opacity={segment.opacity}
              stroke="var(--color-bg)"
              strokeWidth={segment.strokeWidth}
              className="transition-[fill,opacity] duration-300"
            >
              <title>{segment.title}</title>
            </path>
          ) : (
            <path
              key={segment.id}
              d={segment.d}
              fill={segment.fill}
              opacity={segment.opacity}
              stroke="var(--color-bg)"
              strokeWidth={segment.strokeWidth}
              onClick={() => goTo(segment.stepIndex as number)}
              className="cursor-pointer transition-[fill,opacity] duration-300 hover:opacity-100"
            >
              <title>{segment.title}</title>
            </path>
          ),
        )}

        <circle
          cx={DIAL_VIEWBOX / 2}
          cy={DIAL_VIEWBOX / 2}
          r={HUB_RADIUS}
          fill="var(--color-bg)"
        />
      </svg>

      {/* The hub sits over the SVG as real text, so it inherits the type scale
          rather than being sized in user units. */}
      <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center text-center">
        <span className="font-mono text-[9px] font-bold tracking-[0.16em] text-faint uppercase">
          Complete
        </span>

        <span className="text-[26px] leading-none font-extrabold tabular-nums">
          {percent}%
        </span>

        <span
          className="mt-1 font-mono text-[9px] font-bold tracking-[0.1em] uppercase"
          style={{ color: RAG_COLOUR[rag] }}
        >
          {overall === null ? "No effort set" : `Effort ${RAG_LABEL[rag]}`}
        </span>

        <span className="mt-1 font-mono text-[9px] tracking-[0.08em] text-faint tabular-nums">
          {index + 1} / {STEP_COUNT}
        </span>
      </div>
    </div>
  );
}
