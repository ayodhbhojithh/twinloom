import { button, esc, fonts, kicker, palette, plate, rule, shell, step, } from "./shell.js";
/* ---------------------------------------------------------------------------
...
const { SANS } = fonts;
const h1 = (text) => `<h1 style="margin:14px 0 0;font-family:${SANS};font-size:21px;line-height:1.2;font-weight:800;letter-spacing:-0.028em;color:${INK}">${esc(text)}</h1>`;
const p = (text, size = 13.5) => `<p style="margin:12px 0 0;font-family:${SANS};font-size:${size}px;line-height:1.65;color:${BODY}">${text}</p>`;
/**
 ...
 */
export function scopeReceipt({ name, ref, attachments, }) {
    const body = `
    ${kicker("Received")}
    ${h1("We have your scoping request.")}
    ${p(`Thank you, ${esc(name)}. A person reads every one of these - what comes back is a written scope in your own words, within two working days.`)}

    ${plate("Your reference", ref)}

    ${p(attachments > 0
        ? `Quote it in any reply. The ${attachments} ${attachments === 1 ? "file" : "files"} you attached are filed under it.`
        : "Quote it in any reply, and anything you add later is filed under it.", 12.5)}
    ...
    ${p(`Nothing here commits you to anything, and you can reply to this message with anything you forgot.`, 12)}`;
    const text = [
        `Hello ${name},`,
        "",
 ...
 * stops working.
 */
export function bookingConfirmation({ name, meeting, minutes, when, zone, meet, }) {
    const body = `
    ${kicker("Booked")}
    ${h1("The time is yours.")}
    ${p(`Thank you, ${esc(name)}. It is in the diary and a calendar invitation is on its way to this address - accepting it puts the meeting in your own calendar, and moving or cancelling it there tells us straight away.`)}
    ...
    ${p(`<a href="${esc(meet)}" style="color:${INK};font-weight:600;word-break:break-all">${esc(meet.replace(/^https?:\/\//, ""))}</a>`, 13)}`
        : ""}

    ${button("https://twinloom.com/build", "Write down what you want first")}

    ${p("You do not have to. Come with it done or come with nothing - both are a conversation we can have.", 12)}`;
    const text = [
        `Hello ${name},`,
        "",
